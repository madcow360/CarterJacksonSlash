...
\subsection{20 Ways to Show I Love You}
\indent $\sim$ Your task should you choose to accept it, is to perform the following positions in order of appearance, over a 30 day period.
The Positions must be completed in order of appearance. A position is completed when both parties have orgasmed due to the position.\\
The winner is decided at the end of the 30 day period, If:
\begin{itemize}
  \item Every position is complete
  \item Both parties have a favourite position
  \item Both parties can agree to a OTP (One True Position)
\end{itemize} 
	If for any reason the two parties cannot agree upon a position for the allocation of OTP, then those two positions will be repeated, until both parties agree.
	Both Parties after completing this challenge Get the ability to;
\begin{itemize}
  \item Call upon there favourite Position 
  \item Call upon the One True Position
\end{itemize}
The Winner of this Wager is the Party whom at completion, have called upon the wager
 less times than the other party when it is completed, excluding zero.
 See Section \ref{sec:ListFigures} Figure~\ref{fig:20 ways} on page \pageref{fig:20 ways} for listing.
\newpage


\subsection{Alpha as Fuck}
   -  Complete the scenario as per described (TO BE ADDED) 
   
   
\subsection{Kama Sutra}
   $\sim$ Over an Extended Period, an attempt will be made to complete all positions mentioned in the
   Kama Sutra.\\
    $\sim$ An official Checklist is in Section \ref{sec:ListFigures} Figure~\ref{fig:KamaSutra} on page \pageref{fig:KamaSutra}\\
   $\sim$ Should some positions be unable to be completed for flexibility, attempts shall be made to rectify this 
   until all positions completed at least once.\\
   
   
 \subsection{But I AM Working\ldots}
   $\sim$ Danielle and Sam attempt to copulate while otherwise occupied, in the form of television and completing a Quiz.\\
   $\sim$ This challenge is to be completed twice, with one party attempting to focus on work and tv while also engaging in coitus.\\ 
   The quiz is to be compiled by a Third Party, containing Fandom References, General Knowledge, and basic arithmetic.\\
   $\sim$ The winner Gets to utilise their partner as a table for 2 Activities to be decided at a later date. eg food, desk etc. 
   
 \subsection{The Great Roleplay Rumble}
 $\sim$Over an extended period, an attempt will be made to complete all the positions mentioned in Section 
 \ref{sec:ListFigures} Figure~\ref{fig:Roleplay} on page \pageref{fig:Roleplay}
